...
systematics introduced by observing systems.

We propose to use the probabilistic nature of Gaussian Processes 
(specifically, equation \ref{eq:var}) to provide a direct
...
also allows us to project the impact of future observations on the entire data
set so as to maximize the efficacy of our follow-up \cite{Garnett11,Garnett12}.
...
\begin{figure}
\includegraphics[scale=0.2]{becker_RRLyrae_fig.png}
\caption{
Two hundred days of simulated LSST of an RR Lyrae star template, taken from
\cite{rrlyrae}.  The left panel shows sampling at the universal cadence.  The
right panel shows sampling at the deep drilling cadence.
}
\label{fig:RRLyrae}
\end{figure}
...
optimum follow-up observations.
...
it an instrumental artifact?'' \cite{brink2012}.  These methods are all useful
...
by modern, large-data experiments in astronomy and other sciences.
...
intention is to introduce an information-theoretic approach which will allow
us to perform a similar determination in real time as the observations come in,
...
(2011, 2012) who use a the correntropy to determine a light curves true period.
...
i.e. the value of $P(z_\text{photometric}=z_\text{spectroscopic})$ 
as a function
