...
\caption{ Taken from \citet{rrlyrae} the left panels shows the
  expected sampling (for one year of data) of a light curve by the
  LSST at its main survey cadence (top left) and for a higher sampling
  rate that will be employed for 10\% of the LSST survey (bottom
  left). The right panels shows a measure of the how well the light
  curves can be phased (i.e.\ combining the observations while solving
  for the periodicity of the data). This illustrates the significant
  effect on scientific output of more and more targeted data.}
...
\subsection{Active Learning for Transient Classification}   {\bf
  ...
  acknowledged as a necessary support technology for the forthcoming
  age of survey cosmology
  ...
  algorithms that can learn the classification of an object given
  data that are well sampled. For the DES and the LSST the data
  streams are, however, poorly sampled in the temporal
  domain. Figure~\ref{fig:RRLyrae} shows the expected sampling of data
  from the LSST and how, using current techniques, these translates
  into the ability to classify and characterize the light curves of
...
misclassification rate of 22.8\%.  


The limitations of these existing approaches are that they are
optimized for the case of binary classifications: ``Is something a
quasar, or is it not?''  \cite{kim2011,pichara2012}, ``Is an object at
redshift greater than 4 or is it not?'' \cite{morgan2011}, ``Is an
...
full range of classifications. They are also undertaken as a batch or
off-line process (which will not scale to the real-time classification
required for next generation surveys)
...
Examples of this include the use of correntropy as a measure of the
correlation between observed intensities at different time lags in already
observed light curves and the entropy of as-of-yet
untaken observations.  
...
\subsubsection{Caching structures for computational efficiency for
...
constrain the underlying cosmological model. We will address these
...
An extension called TCube stores time series in the AD-Tree
...
\vspace{-4mm}
\paragraph{N-trees.}  The use of multiple trees simultaneously was demonstrated
to yield orders of magnitude speedups when computing n-point correlations
\cite{gry:nbdy,kubicaKDD2005}.  At its core, these amount to template
matching problems and we plan to generalize these for pattern discovery
with missing data.
