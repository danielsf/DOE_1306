\documentclass[prd,nofootbib,floatfix,11pt,tightenlines]{revtex4}
...
learning algorithms.  Our prototype implementation using a classical
...
function representation of the function models. A preliminary analysis using
 principal component
...
\item It is clear that we can improve performance by looking ahead further
  (e.g. see \citet{Garnett12}).  We propose to develop pruning rules that
...
machine learning algorithms will face (see Section~\ref{sec:sparse}.  We will demonstrate our novel
...
considering an entire pool of test objects, they appear one at a time as
they are detected and the algorithm must decide whether and how to follow
up on each immediately as they are detected.   Given the size and dimensionality of data from the next generation of
...
\item Develop GP algorithms that are robust against sparse training data and
capable of modeling multi-modal probability
distributions.  Build a photometric redshift algorithm based on these next generation
GPs.  Test the algorithm against data from the SDSS, DES, and simulated LSST.

\item Develop active learning methods optimized for our next generation GPs.
...
one- and two-step-look-ahead criteria.  We will test these algorithms in the case of
photometric redshifts.

\item Develop a software package that provides access to our next generation GPs for
...
\item Develop active feature acquisition algorithms based on the probability distributions
...
\item Extend our next generation GPs to the problem of 
...
\item Extend the active feature acquisition algorithms developed for photometric
...
\item Generalize our active learning and active feature acquisition algorithms so that
...
\item Integrate our active learning and active feature acquisition algorithms to
...
\item Develop a software package that will provide access to all of our
...
\bibitem[Connolly {\it et al}. 2010]{connolly10}
...
\bibitem[Gray {\it et al}. 2004]{GrayMoore}
...
\bibitem[Kubica {\it et al}. 2007]{kubica}
