...
are obtained through labeling by human experts and/or additional data
...
improved classification and online follow-up of transient astronomical
objects (see Section \ref{sec:transients}).  We will release our software
...
classifying astronomical objects \cite{vdp2009,daniel2011} as well as an
...
from its observed photometry (usually in five or six bands).  Because of
the cost, only a few spectrographically determined redshifts are available
...
gives an  improvement on the peformance of
...
\subsubsection{Transients: Additional Challenges}
...
Transient classification and active learning must be done in real
time.  Where an object's spectrographic redshift could be measured years
after its photometry was collected, a decision to follow up on a transient
must be taken in seconds or minutes. Otherwise, valuable transient data
might pass before it can be captured.  With timescales as short as seconds
...
Finally, small errors in the identification and classification of transient
...
available.  While we focus our demonstration and testing on astronomical
...
tree in fig. \ref{fig:searchtree}.  The difference is that the evaluation
...
\item In \cite{Wang13} we showed how to combine an exact one-step myopic
  ...
  to achieve results better than expensive look aheads on graph problems.
