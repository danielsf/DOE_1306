...
We propose to address these problems by using Gaussian Processes (GPs) to
model the probability distributions underlying the data.
...
The proposed work in the rest of this section will be based on Gaussian
processes.  We suggest methods of handling non-unimodal, posterior
distributions and consider especially the scenario where a large gap is
present in the training data, though not in the intended test data.  We
propose the application of GPs for anomaly detection and classification.
In the following section, we discuss active learning.  The algorithms will
be described as they would be built on GPs, but are general enough that they
may be used on any model that provides full predictive distributions.
