...
{\bf Science Outcomes.}  Data from the Dark Energy Survey will become publically
available over the term of this project.  We hope to use the algorithms
developed above to achieve significant improvement in the determination of
photometric redshifts (explained in Section \ref{sec:photoz} below) and
classification of transient astronomical objects.  We also hope to initiate a
dialog whereby our algorithms may be spread to other science -- such as
climatology and microbiology -- wherein complex systems can be modeled 
or measured at great
expense and algorithms are required to determine which models
and experiments are actually worth performing.
...
terminology above).
We describe how we will meet this challenge below.
...
plotted in Figure \ref{subfig:gp}.  Note all of the high
...
\begin{itemize}
\item (1) Fit the training data to a single-mode GP.

\item (2) Use Bayesian model selection \cite{mackay}
to compute the model likelihood of this single-mode GP.

\item (3) Divide the training data into two sub-populations based on their output
values.

\item (4) Fit each sub-population to its own GP.

\item (5) Compute the model likelihood of this two-mode model and compare to the
model likelihood of the single-mode model.

\item (6) Repreat steps 3-5 for three-mode, four-mode, etc. models until
satisfied that you have found the best fit to the training data.
\end{itemize}
Through this iterative approach, we hope to build a GP such that, even when it
...
This is just a toy model.  The learning algorithm illustrated operates in a high
signal-to-noise regime.  It takes no account of the relative cost of doing one
measurement or another.  No gaps exist in the available training data.
These are all challenges that real-world machine learning algorithms will face.
We propose to develop algorithms to overcome them.
...
Current state-of-the art regression algorithms attempt to learn a one-to-one
relationship between the input data and the output function. Uncertainties are
also learned, but these are usually heuristics derived by considering multiple
attempts at regressionl such as by a committee of aritficial neural networks
\cite{annz}.  We propose to replace this framework with one that directly models
the probability distributions underlying the data.
We believe that this framework
is the most robust and informative available.  It will allow us
...
distribution \cite{gp}.  Given a set of observed samples from the function's
data space,
it can make predictions about a set of other locations and assign these
predictions a multivariate Gaussian distribution.  An important
feature of GPs is that they do not make parametric assumptions about the
form of the function they are modeling and thus are well suited to
nonlinear regression problems.
...
in the case of sparse measurements.  Examples in astrophysics include the
expansion history of the universe \cite{ericgp} and interpolating point
spread functions across large images \cite{psf}.  Mahabal {\it et al}. 2008b and
Wang {\it et al}. (2011,
2012) use a mixture of Gaussian Processes to model light curves and
identify periodically varying stars.  Huisje {\it et al}. (2011, 2012)
improve upon their methods, using information-theoretical quantities to
...
= \exp(-\frac{1}{2}|\vec{\theta}^{i} - \vec{\theta}^{j}|^2/\ell^2)$ where $\ell$
is a characteristic length scale set by cross-validation.  
Under those
assumptions, one can derive a posterior probability distribution for $f$
at a new query point $\{\vec{\theta}_{q}\}$ by marginalizing over
...
The thin curves in Figure \ref{fig:desert} show that Gaussian Processes
...
is signitifcantly greater for a Gaussian Process than current state of the
art classification and regression algorithms.
...
Gaussian Process models and guide the active learning selection
...
used.  Foster {\it et al}. (2009) advocate an approximate Gaussian Process method
...
given unknown $\{\vec{\theta}_q\}$, the GP is only given the $k$ nearest
...
and the one which minimizing the entropy of the output probability distribution
...
Process classifier by considering the mean value of $\ln[P(\text{truth})]$
...
based on the output covariance matrix of equation \ref{eq:cov} which will allow
us to perform a similar determination in real time as observations are made,
directing experiments as they are performed.  An 
