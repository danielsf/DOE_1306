...
approaches but at a substantial cost in performance.
...
most information about the unknown datum.  We propose to extend our
...
we can choose to observe or ignore.  In the DLA work, a different
...
identified. Starting from the myopic algorithms in our earlier work we
propose to develop {\bf something here}
% We
...
The next generation of astrophysical surveys will open the time
domain by visiting the same region of sky many thousands of times over
a period of ten years\cite{tyson}. This temporal information offers
the potential to discover new classes of physical phenomena while
coming with many associated computational challenges. Variability
within the universe is believed to be present on time scales of
seconds to hundreds of years. The shortest time scales correspond to
the explosion of the most massive stars within the universe which
produce short but intense optical and gamma-ray flashes.  Intermediate
timescale variability comes in the form of supernovae (SNe) which
detonate, brighten and then dim. Long term variability comes from the
fluctuations in the brightness of AGNs with timescales of over a year.
%These standard
%%candles can be used to determine the rate at which the universe
%expands and thereby measure its mass and energy content
%\cite{perlmutter99}. 

%Surveys, such as the LSST, will survey%
%half the sky every three nights detecting 250,000 SNe per year.

% increasing the
%accuracy of measures of the energy content of the universe by an order
%of magnitude.

To identify, classify and report anomalies in time for followup
observations, detection and classification must be undertaken in
almost real-time (e.g.\ within 60 seconds of an LSST observation). We
require these classifications to be probabilistic, to incorporate
uncertainties about the classification and to have the ability for
algorithms to learn based on {\it a posteriori} information from
earlier classifications.
% It
%must be able to predict what additional information might be needed to
%improve (or exclude) the likelihood of a given classification and to
%specify which parameters led to the source being classified as
%anomalous. 
Small errors in the identification and classification of these
anomalous sources will swamp any underlying signal. The LSST will
detect 7.5x10$^8$ sources {\bf every night}. Even for the most
numerous transient events (SNe), with 250,000 identified a year, this
detected on an image. For the most energetic bursters the magnitude of
the challenge is 500-times larger.  Algorithms for identifying
anomalies and variability must, therefore, be robust to false
positives and missing data
% and the fact that we observe sources at different
%wavelengths at different times 
and must account for the cadence in how
we sample the time domain and variations in the quality of the data
due to atmospheric conditions.
% as well as changes in the
%performance of the telescope and camera.
